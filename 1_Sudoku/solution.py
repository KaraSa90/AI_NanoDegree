...
diag2 = [r + c for r,c in zip(rows, reversed(cols))]
...
def naked_twins(values):
    ...
    # identify potential twins
    potential_twins = [box for box in values.keys() if len(values[box]) == 2]
    # find twins by comparing values
    naked_twins = [[box1,box2] for box1 in potential_twins \
                    for box2 in peers[box1] \
                    if set(values[box1])==set(values[box2]) ]
    # For each pair of naked twins,
    for box1, box2 in naked_twins:
        peers1 = set([box for box in peers[box1] if box != box2 and len(values[box]) > 1])
        peers2 = set([box for box in peers[box2] if box != box1 and len(values[box]) > 1])
...
def eliminate(values):
    ...
    solved_values = [k for k in values.keys() if len(values[k]) == 1]
