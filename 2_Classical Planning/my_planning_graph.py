...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        for precondA in actionA.preconditions:
            if ~precondA in actionB.effects:
                return True
        for precondB in actionB.preconditions:
            if ~precondB in actionA.effects:
                return True
        return False
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for actionA in actionsA:
            for actionB in actionsB:
                if not actionLayer.is_mutex(actionA,actionB):
                    return False
        return True
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        goals = self.goal
        cost = [float("inf")]*(len(goals))
        i = 0
        while not self._is_leveled:
            goalsMet = True
            for goal,k in zip(goals,range(len(goals))):
    ...
    def h_setlevel(self):
            ...
            if goalsMet:
                ...
                for goalA in self.goal:
                    for goalB in self.goal:
                        if goalB != goalA and literal_layer.is_mutex(goalA,goalB):
                            goalsMutex = True
                if not goalsMutex:
                    return i
            i += 1
        ...
        while not self._is_leveled:
            ...
            if goalsMet:
                ...
                for goalA in self.goal:
                    for goalB in self.goal:
                        if goalB != goalA and self.literal_layers[-1].is_mutex(goalA,goalB):
                            goalsMutex = True
                if not goalsMutex:
                    return i
            self._extend()
