    ...
    **********************************************************************
    ...
    def get_action(self, state):
        ...
        **********************************************************************
        ...
        if True: # Set True for iterative deepening with alpha_beta
            # set my_heuristic to True to use advanced heuristic
            self.id_alpha_beta(state, depth_limit = 20, my_heuristic = False)
        if False: # Set True for MCTS
            self.monte_carlo_tree_search(state, 2**(1/2))

    def monte_carlo_tree_search(self, state, c = 1):
        statistics = defaultdict(lambda: np.array([0, 0])) #[X_j, n_j]
        # sub functions
        ...
        def update(statistics, nodes, result):
            for node in reversed(nodes):
                statistics[node] += np.array([result, 1])
                result = - result
            return statistics
